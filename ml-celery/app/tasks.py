...
app = Celery(
    celery_config.QUERY_NAME, broker=config.BROKER, backend=config.REDIS_BACKEND
)
...
def object_detection_task(self, task_id: str, data: bytes):
    ...
    try:
        image = ml_image_helper.read_image_from_path_to_numpy(
            data["upload_result"]["path"]
        )
        ...
        for j in range(len(detection_boxes)):
            ...
            image_draw = cv2.rectangle(
                image_draw, (xmin, ymin), (xmax, ymax), box_color, 1
            )
            cv2.putText(
                image_draw,
                obj["class_name"],
        ...
        data["detection_draw_url"] = (
            "http://{}:{}/api/v1/show-image/?path_image=".format(
                config.BE_HOST, config.BE_PORT
            )
            + celery_config.ML_STORAGE_OBJECT_DETECTION_PATH
            + string_time
            + "/"
            + str(task_id)
            + celery_config.ML_IMAGE_TYPE
        )
