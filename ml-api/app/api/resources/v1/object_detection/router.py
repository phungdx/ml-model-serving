...
from api.entities.object_detection import (
    MlTimeHandle,
    MlResult,
    MlStatusHandle,
    MlResponse,
...
async def ml_process(
    *,
    ...
    if file.content_type not in ["image/jpeg", "image/png"]:
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail="file_type not support!"
        )
    time = time_helper.now_utc()
    task_id = str(
        uuid.uuid5(uuid.NAMESPACE_OID, config.ML_QUERY_NAME + "_" + str(time))
    )
    ...
    return MlResponse(
        status="PENDING", time=time, status_code=HTTP_200_OK, task_id=task_id
    )
...
def ml_status(
    *,
    ...
    if data == None:
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail="task id not found!"
        )
